...
namespace Mygame.Editor
{
    ...
    public class RecipeScriptableObjectEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                //Debug.Log("Change at quantity");

                if (serializedObject.FindProperty("quantityItemOutput").intValue <= 0)
                {
                    Debug.LogError("Output crafting item quantity do not allow <= 0.");
                    serializedObject.FindProperty("quantityItemOutput").intValue = 1;
                    throw new System.Exception();
                }

                if (serializedObject.FindProperty("quantityItemOutput").intValue > recipe.outputItem.max_quantity)
                {
                    Debug.LogError("Output crafting item quantity do not allow > max stacked item quantity.");
                    serializedObject.FindProperty("quantityItemOutput").intValue = 1;
                    throw new System.Exception();
                }
            }
